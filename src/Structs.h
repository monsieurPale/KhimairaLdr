...
typedef HRESULT(WINAPI* fnCoInitializeEx)(
	LPVOID pvReserved,
	DWORD  dwCoInit
);
...
typedef void(WINAPI* fnCoUninitialize)();

typedef void(WINAPI* fnCoTaskMemFree)(
	_Frees_ptr_opt_ LPVOID pv
);

typedef DWORD(WINAPI* fnGetEnvironmentVariableW)(
	LPCWSTR lpName,
	LPWSTR  lpBuffer,
	DWORD   nSize
);
...
typedef DWORD(WINAPI* fnGetFileAttributesW)(
	LPCWSTR lpFileName
);
